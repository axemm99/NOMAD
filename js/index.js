...
addLoadEvent(function() {
    ...
    // FORMS
        ...
        frmRegister.addEventListener("submit", function(event) {
            let newUser = new User(modalRegisterName.value, modalRegisterEmail.value, modalRegisterPassword.value, modalRegisterPhoto.value, 2, 0)
...
/*
    ...
    function addOptionsToSearchBar(){
        let srchBooks = document.getElementById("srchBooks")
        let btnSearch = document.getElementById("btnSearch")
    
        let optnSearch
    
        for (let i = 0; i < books.length; i++) {
            optnSearch = `<option value ='${books[i].bookTitle}'></option>`
        }
    
        srchBooks = srchBooks.value
    
        btnSearch.addEventListener("submit", function(event){
            for (let i = 0; i < books.length; i++) {
                if(srchBooks == books[i].bookTitle){
                    bookPage()
                }            
            }
        })
