...
/* user */
class User {
    constructor(userName, userEmail, userPassword, userPhoto, userPermissions, fineValue) {
    ...
    static calculateFineValue(timeDays){
        let config = configGeneral

        if (timeDays > config[0]) {
            let totalDays = seconds - parseInt(config[0])
            let totalFineValue = parseInt(config[1]) * totalDays

            if (totalFineValue >= parseInt(config[2])) {
                totalFineValue = parseInt(config[2])
            }
            console.log(totalFineValue)
        }
        else {
            console.log("Livro entregue com sucesso!")
        }
    }

    // CALCULATE TIME WITH BOOK                          ??????????????????????????????
    /*static timeAcumulation(timeDays) {
        let today = new Date();
...
class Book {
    ...
    static requestBookById(id) {
        for (let i = 0; i < books.length; i++) {
            if(books[i].id == id) {
                let newRequest = new Request(userCurrent, id, Request.getRequestDate, "")
                checkRequestValid(id, newRequest)
            }
        }
    }
}
...
class Request {
    ...
    // GET REQUEST DATE BY BOOK ID
    static getRequestDateByBookId(id) {
        for (let i = 0; i < requests.length; i++) {
            if (requests[i].bookId == id && requests[i].userId == userCurrent) {
                return requests[i].requestDate
            }
        }
    }

    // GET DELIVERY DATE BY BOOK ID
    static getDeliveryDateByBookId(id) {
        for (let i = 0; i < requests.length; i++) {
            if (requests[i].bookId == id && requests[i].userId == userCurrent) {
                return requests[i].deliveryDate
            }
        }
    }
}
...
class Wishlist {
    ...
    static removeWishlistPreferencesByCategoryId(id) {
        let tempArray = []
    ...
    static removeWishlistPreferencesByTagId(id) {
        let tempArray = []
    ...
    // REMOVE WISHLIST TAG PREFERENCE BY USER ID
    static removeWishlistPreferencesByLibraryId(id) {
        let tempArray = []
    ...
    static editWishlistCategoriesByUserId(id) {
        for (let i = 0; i < wishlists.length; i++) {
            if (wishlists[i].userId == id) {
                wishlists[i].notificationsCategories = wishlistCategories
            }
        }
    }
    ...
    static editWishlistTagsByUserId(id, tags) {
        for (let i = 0; i < wishlists.length; i++) {
            if(wishlists[i].userId == id) {
                wishlists[i].notificationsTags = wishlistTags
            }
        }
    }
    ...
    static editWishlistLibrariesByUserId(id, tags) {
        for (let i = 0; i < wishlists.length; i++) {
            if(wishlists[i].userId == id) {
                wishlists[i].notificationsLibraries = wishlistLibraries
            }
        }
    }
}
...
//////////////////////////
    ...
    /*
    // GET CITY NAME BY PARISH ID
    static getCityByParishId(id) {
        for (let i = 0; i < libraries.length; i++) {
            if (libraries[i].parish == id) {
                return libraries[i].city
            }                
        }
}*/
...
if (!localStorage.books) {
    ...
    books.push(book06)
    book07 = new Book("Odisseia de Homero", "Frederico Lourenço", "ClaroEnigma", "2018", 688, 2, [6, 11], "Aceitável", "Alexandre", "2017-04-01", "https://images.livrariasaraiva.com.br/imagemnet/imagem.aspx/?pro_id=4238174&qld=90&l=430&a=-1", "A Odisseia não é apenas um dos grandes épicos da literatura grega; é também um dos pilares do cânone ocidental, um poema de rara e extraordinária beleza - e o livro que mais influência exerceu, ao longo dos tempos, no imaginário ocidental.", [0, 5, 5, 3], 3)
    books.push(book07)
